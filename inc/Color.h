...
inline LabColor rgbToLab(uint8_t r, uint8_t g, uint8_t b) {
    float R = linearize(r), G = linearize(g), B = linearize(b);
...
inline float deltaE(const LabColor& a, const LabColor& b) {
    float dL = a.L - b.L, da = a.a - b.a, db = a.b - b.b;
    return std::sqrt(dL*dL + da*da + db*db);
}

static const NamedColor COLOR_DB[] = { // color names courtesy of the xkcd color name survey, see https://xkcd.com/color/rgb/ for more information
...
inline std::string getColorName(uint8_t r, uint8_t g, uint8_t b) {
    LabColor input = rgbToLab(r, g, b);
    float bestDist = std::numeric_limits<float>::max();
    const char* bestName = "unknown";

    for (const auto& c : COLOR_DB) {
        LabColor ref = rgbToLab(c.r, c.g, c.b);
        float d = deltaE(input, ref);
        if (d < bestDist) {
            bestDist = d;
            bestName = c.name;
        }
